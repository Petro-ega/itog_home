...
def general():
    ...
    if form.validate_on_submit():
        ...
        return (
            f'Name: {name} <br> Password: {generate_password_hash(password)} <br> '
            f'Remember me: {remember_me} <br> Salary: {salary} <br> Gender: {gender} <br> '
            f'Country: {country} <br> Message: {message} <br> Photo: {photo}'
        )
